...
U32 g_u32_all_file_Count = 0;//all input number
...
#define CLOCK_GROUP_WIDTH 		200
#define CLOCK_GROUP_NUM 		30
...
int Read_trace_file(char* pcFileName)
{
	...
	//printf("Read a total %d records from %s\n", u32LineCount, pcFileName );

	// 3.(b) print out the total number of prefixes in the input file
	/*
	printf("There are total %d prefixes in the %s\n\n", u32LineCount, pcFileName );

	if( u32LineCount == 0 )
	{
		printf("[ERROR] No data in Trace_file file(%s)!!\n", pcFileName);
		return -1;
	}
	*/
...
int PreFix_insert(U32 u32IP, U8 u8Len)
{
	...
	/*
	else if( pstPreFix_insert->u32_ip <= g_ppstPreFixHead[u32GroupIdx]->u32_ip ) // current prefix is small then head
	{
...
int Compute_segment(U8 u8_d)
{
	...
	for( i = 0; i < u32TotalPreFixes; i += 1)
	{
		if( g_ast_PreFix[i].u8_len >= u8_d )
		{
			...
			//g_ast_PreFix[i].u32GroupIdx_ = u32GroupIdx;
			//printf("(%5d) ip:0x%08X, 0x%08X \n", i+1, ast_PreFix[i].u32_ip, u32GroupIdx );
		...
		{
			//g_arr_specialPreFix[i] = g_ast_PreFix[i].u32_ip;
			g_arr_specialPreFix[u32SpecialGroupCount] = g_ast_PreFix[i].u32_ip;
            //printf("(%d) 0x%08X => %d\n", i, g_ast_PreFix[i].u32_ip, u32SpecialGroupCount );

			u32SpecialGroupCount += 1;
		}
	}

    //printf("u32SpecialGroupCount = %d\n", u32SpecialGroupCount);
...
int main(int argc, char *argv[]) {
	...
	// read all the prefixes from the input file
	//begin = rdtsc_64bits();
	{
	...
	// 3.(b) print out the total number of prefixes in the input file
	g_u32_all_file_Count = g_u32PreFixesCount + g_u32_Inserted_PreFix_Count + g_u32_Deleted_PreFix_Count + g_u32_Trace_file_Count;
	printf("The total number of prefixes in the input file is : %d.\n", g_u32_all_file_Count);

	Compute_length_distribution();

	//printf("\n===============================================================\n");

	i_rtn = Compute_segment(8);
	if( i_rtn != 0 )
	{
...
# endif
	...
	//Print_PreFixes(g_ppstPreFixHead[0xC9]);
